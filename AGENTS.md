...
- Rector (`rector.php`) enforces PHP 8.3 + dead-code/code-quality/coding-style/type-declaration/naming sets, with a specific skip list - check it before assuming a refactor rule applies.

## Updating from the template

This project was generated from the [Scaffold](https://github.com/AlexSkrypnyk/scaffold) template and can pull the template's latest infrastructure (CI workflows, linting and test configuration) at any time. When you are asked to "update scaffold", fetch the updater skill from the template repository into the local `.claude/skills/` directory, then invoke it:
